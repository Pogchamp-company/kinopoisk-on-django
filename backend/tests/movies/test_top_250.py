...
class TestTop250:
    def test_existing_movie(self, client):
        ...
        response = client.get(reverse('movies.top250', kwargs=dict(movie_type=movie_type.title)))
        assertTemplateUsed(response, 'movies/top_250.html')
        assert response.status_code == 200

    def test_not_existing_movie(self, client):
        assert client.get(reverse('movies.top250', kwargs=dict(movie_type='qknwjxgnwqzuilzwqh;dn'))).status_code == 404
